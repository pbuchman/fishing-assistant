...
export function requiredEnvFor(env: NodeJS.ProcessEnv = process.env): string[] {
  void env;
  return [...requiredEnv, 'FA_OPENROUTER_APP_API_KEY', 'FA_MINIMAX_APP_API_KEY'];
}

export function validateRequiredEnv(env: NodeJS.ProcessEnv = process.env): void {
  validateHttpServerRequiredEnv(requiredEnvFor(env), env);
}
