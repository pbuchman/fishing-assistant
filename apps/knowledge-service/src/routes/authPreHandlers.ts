...
function logAuthFailure(
  ...
  input: {
    event: string;
    routeGroup: 'knowledge-admin' | 'knowledge-user';
    reason: string;
    statusCode: number;
    accountState?:
...
export async function requireApprovedAdminAuth(
  ...
  const resolved = await resolveApprovedAuthorization(request, reply, 'knowledge-admin');
  ...
  request.faApprovedAuthorization = resolved;
  return undefined;
}
...
export async function requireApprovedUserAuth(
  ...
  const resolved = await resolveApprovedAuthorization(request, reply, 'knowledge-user');
